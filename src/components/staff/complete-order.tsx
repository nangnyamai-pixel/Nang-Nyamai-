...
export function CompleteOrder({ orderId }: { orderId: string }) {
  ...
  return <main className="status-page"><Link className="details-back" href="/staff/orders">← <span>Back to Order Queue</span></Link><header className="status-page-header"><p>Payment Processing</p><h1>Complete Order</h1></header><section className="details-panel kitchen-confirmation"><span className="empty-icon">✓</span><h2>Order Completed!</h2><div className="status-summary"><div><small>Order ID</small><strong>#{order.order_number}</strong></div><div><small>Table</small><strong>{order.restaurant_tables?.table_number ?? "—"}</strong></div><div><small>Completed Time</small><strong>{new Date(order.updated_at).toLocaleString("en-MY", { timeZone: "Asia/Kuala_Lumpur" })}</strong></div><div><small>Total Paid</small><strong>{money.format(Number(order.total))}</strong></div><div><small>Payment Method</small><strong>{order.payment_method === "cash" ? "Cash" : "Card"}</strong></div></div><Link className="details-primary-link" href={`/staff/payments/${orderId}/receipt`}>View Receipt</Link><Link className="details-primary-link" href="/staff/orders">Back to Order Queue</Link></section></main>;
}
