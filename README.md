...
**Sprint 0 ΓÇö environment & architecture.** No application screens beyond
...
| Auth | Supabase Auth ΓÇö Guest + Google OAuth (customer), email/password (staff/admin) |
...
src/
  app/
    (customer)/order/   # table QR landing ΓåÆ menu ΓåÆ cart ΓåÆ order (Sprint 1)
  ...
  lib/
    ...
    utils/
  hooks/
  types/         # hand-written DB types (Sprint 0) ΓåÆ generated later
...
Defined in `.env.example` ΓÇö copy to `.env.local` and fill in real values
...
server-side code ΓÇö never in a Client Component.
...
3. Review and run `supabase/sql/001_schema.sql` in the SQL editor (proposed
   schema ΓÇö read it first, it is not applied automatically by this repo).
4. Review and run `supabase/sql/002_rls_policies.sql` (proposed RLS
   policies ΓÇö also for review before running).
...
3. In Supabase Dashboard ΓåÆ Authentication ΓåÆ Providers ΓåÆ Google: paste the
   Client ID and Client Secret, enable the provider.
4. In Supabase Dashboard ΓåÆ Authentication ΓåÆ URL Configuration, add your app's
   callback URL, e.g. `http://localhost:3000/auth/callback` (dev) and the
   production equivalent.

**Flow:** customer at Table T12 ΓåÆ taps "Continue with Google" ΓåÆ Google
login ΓåÆ redirected to `/auth/callback` (`src/app/auth/callback/route.ts`)
ΓåÆ session established, profile synced with `role: customer` ΓåÆ redirected
...
- Route-level protection: `middleware.ts` redirects unauthenticated users
  away from `/staff/*` and `/admin/*`. This is a UX convenience only ΓÇö
...
- Customers can create their own orders but cannot update order status or
  ...
  fields) ΓÇö only staff/admin can transition order status.
...
browses the menu ΓåÆ orders Sarawak Laksa ΓåÆ staff verifies ΓåÆ preparing ΓåÆ
ready ΓåÆ customer sees each update in real time ΓåÆ staff confirms payment
and completes the order.

Each sprint stops for review/approval before the next one starts.

Local test change for Git push verification.
